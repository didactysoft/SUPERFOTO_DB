...
def obtener_conexion():
    ...
    try:
        # Aseguro que la carpeta de la base de datos exista
        os.makedirs(os.path.dirname(DB_PATH), exist_ok=True)
        
        conn = sqlite3.connect(DB_PATH)
        
        # Activamos las llaves foráneas para mantener la integridad
        conn.execute("PRAGMA foreign_keys = ON;") 
        
        # --- Mensaje de conexión exitosa ---
        print(f"Conexión exitosa a la base de datos: {os.path.basename(DB_PATH)}")
