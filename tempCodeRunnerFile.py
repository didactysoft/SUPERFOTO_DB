...
def inicializar_apariencia():
    ...
    # Por defecto usamos Light si el archivo no existe
    tema_para_aplicar = "Light" 
    
    if os.path.exists(archivo_config):
        try:
            with open(archivo_config, "r", encoding="utf-8") as f:
                datos = json.load(f)
                # Buscamos el tema guardado
                tema_para_aplicar = datos.get("tema", "Light")
        except:
            pass # Si hay error en el JSON, mantenemos el default
            
    # ESTO APLICA EL TEMA A TODA LA APP, ESTÉS O NO EN EL MÓDULO CONFIG
    ctk.set_appearance_mode(tema_para_aplicar)

# --- LLAMAR A LA FUNCIÓN ANTES DE ARRANCAR ---
...
"""
    ...
    def create_sidebar(self):
        ...
        logo_sf2.grid(row=11, column=0, padx=20, pady=(20, 0))
